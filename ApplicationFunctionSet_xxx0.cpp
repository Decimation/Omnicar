...
void ApplicationFunctionSet_SmartRobotCarLinearMotionControl(SmartRobotCarMotionControl direction, uint8_t directionRecord, uint8_t speed, uint8_t Kp,
															 uint8_t                    UpperLimit);
...
*/
static void ApplicationFunctionSet_SmartRobotCarLinearMotionControl(SmartRobotCarMotionControl direction, uint8_t directionRecord, uint8_t speed,
																	uint8_t                    Kp, uint8_t        UpperLimit)
{
	...
	if (en != directionRecord || millis() - is_time > 10) {
		AppMotor.DeviceDriverSet_Motor_control(/*direction_A*/ direction_void, /*speed_A*/ 0,
															   /*direction_B*/ direction_void, /*speed_B*/ 0, /*controlED*/
...
static void ApplicationFunctionSet_SmartRobotCarMotionControl(SmartRobotCarMotionControl direction, uint8_t is_speed)
{
	...
	switch (direction) {
		...
		Forward:
			...
			if (Application_SmartRobotCarxxx0.Functional_Mode == TraceBased_mode) {
				AppMotor.DeviceDriverSet_Motor_control(/*direction_A*/ direction_just, /*speed_A*/ speed,
																	   /*direction_B*/ direction_just, /*speed_B*/ speed, /*controlED*/
																	   control_enable); //Motor control
		...
		case /* constant-expression */ Backward:
			...
			if (Application_SmartRobotCarxxx0.Functional_Mode == TraceBased_mode) {
				AppMotor.DeviceDriverSet_Motor_control(/*direction_A*/ direction_back, /*speed_A*/ speed,
																	   /*direction_B*/ direction_back, /*speed_B*/ speed, /*controlED*/
																	   control_enable); //Motor control
		...
		case /* constant-expression */ LeftForward:
			...
			directionRecord = 5;
			AppMotor.DeviceDriverSet_Motor_control(/*direction_A*/ direction_just, /*speed_A*/ speed,
																   /*direction_B*/ direction_just, /*speed_B*/ speed / 2, /*controlED*/
																   control_enable); //Motor control
			break;
		case /* constant-expression */ LeftBackward:
			...
			directionRecord = 6;
			AppMotor.DeviceDriverSet_Motor_control(/*direction_A*/ direction_back, /*speed_A*/ speed,
																   /*direction_B*/ direction_back, /*speed_B*/ speed / 2, /*controlED*/
																   control_enable); //Motor control
			break;
...
static void CMD_Lighting(uint8_t is_LightingSequence, int8_t is_LightingColorValue_R, uint8_t is_LightingColorValue_G,
	...
	if (Application_SmartRobotCarxxx0.Functional_Mode == CMD_CarControl_TimeLimit) //enter time-limited control mode
	{
		...
		if (is_Timer != 0) //#1 if the pre-set time is not ... (zero)
		{
			if ((millis() - Application_SmartRobotCarxxx0.CMD_CarControl_Millis) > (is_Timer)) //check the timestamp
			{
				...
				Application_SmartRobotCarxxx0.Functional_Mode =
					CMD_Programming_mode; /*set mode to programming mode<Waiting for the next set of control commands>*/
...
*/
void ApplicationFunctionSet::CMD_LightingControlTimeLimit_xxx0(uint8_t  is_LightingSequence, uint8_t     is_LightingColorValue_R,
															   uint8_t  is_LightingColorValue_G, uint8_t is_LightingColorValue_B,
...
/*
  ...
  No time limited
*/
void ApplicationFunctionSet::CMD_LightingControlNoTimeLimit_xxx0(uint8_t is_LightingSequence, uint8_t     is_LightingColorValue_R,
																 uint8_t is_LightingColorValue_G, uint8_t is_LightingColorValue_B)
{
	static boolean LightingControl = false;
...
void ApplicationFunctionSet::CMD_ClearAllFunctions_xxx0(void)
{
	...
	if (Application_SmartRobotCarxxx0.Functional_Mode ==
		CMD_ClearAllFunctions_Programming_mode) //Command:N110 Clear all functions and enter programming mode
	{
		ApplicationFunctionSet_SmartRobotCarMotionControl(stop_it, 0);
		FastLED.clear(true);
		AppRBG_LED.DeviceDriverSet_RBGLED_xxx(0 /*Duration*/, NUM_LEDS /*Traversal_Number*/, CRGB::Black);
	...
	if (true == IRrecv_en) {
		switch (IRrecv_button) {
			...
			case /* constant-expression */ 9:
				/* code */ if (Application_SmartRobotCarxxx0.Functional_Mode ==
					TraceBased_mode) //Adjust the threshold of the line tracking module to adapt the actual environment
				{
	...
	Serial.println(SerialPortData);
		...
		} else if (!error) //Check if the deserialization is successful
		{
			...
			switch (control_mode_N) {
				...
				case 2:                                                                     /*<Command：N 2> */
					Application_SmartRobotCarxxx0.Functional_Mode =
						CMD_CarControl_TimeLimit; /*Car movement direction and speed control：Time limited mode*/
				...
				case 3:                                                                       /*<Command：N 3> */
					Application_SmartRobotCarxxx0.Functional_Mode =
						CMD_CarControl_NoTimeLimit; /*Car movement direction and speed control：No time limited mode*/
					CMD_is_CarDirection = doc["D1"];
					CMD_is_CarSpeed     = doc["D2"];
#if _is_print
					Serial.print('{' + CommandSerialNumber + "_ok}");
				...
				case 110:                                                                                 /*<Command：N 110> */
					Application_SmartRobotCarxxx0.Functional_Mode =
						CMD_ClearAllFunctions_Programming_mode; /*Clear all function:Enter programming mode*/
#if _is_print
					Serial.print('{' + CommandSerialNumber + "_ok}");
#endif
					break;
